...
async def get_latest_commit(branch: str = None) -> Optional[Dict]:
    endpoint = "commits?per_page=1"
    if branch:
        endpoint = f"commits?sha={branch}&per_page=1"
...
async def get_branches() -> List[Dict]:
    data = await github_api_request("branches?per_page=50")  # Увеличиваем лимит
    return data if data else []


def format_commit_notification(commit: Dict, branch: str = None) -> str:
    ...
    if branch and branch != "main":
        base_text = base_text.replace("Новый коммит", f"Новый коммит в ветке '{branch}'")
